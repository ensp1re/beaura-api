...
import { AuthLoginDto, AuthSignUpDto, ChangePasswordDto, ForgotPasswordDto, ResetPasswordDto } from './dto/auth.dto';
...
export class AuthController {
  ...
  async googleAuth(@Body('idToken') idToken: string) {
    try {
      const decodedToken: DecodedIdToken = await firebaseAdmin.auth().verifyIdToken(idToken);
  ...
  async register(@Body() dto: AuthSignUpDto): Promise<IAuthData> {
    try {
      console.log('dto', dto);
  ...
  async login(@Body() dto: AuthLoginDto): Promise<IAuthData> {
  ...
  async logout(@Res({ passthrough: true }) res: Response): Promise<{ message: string; result: boolean }> {
  ...
  async getNewTokens(@Body() refreshToken: { refreshToken: string }) {
    try {
      if (!refreshToken) {
  ...
  async verifyEmail(@Query('token') emailVerificationToken: string, @Res() res: Response) {
    try {
  ...
  async resetPassword(@Body() data: ResetPasswordDto, @Query('token') token: string, @Res() res: Response): Promise<Response> {
    try {
  ...
  async changePassword(@Body() data: ChangePasswordDto, @Req() req: Request, @Res() res: Response): Promise<Response> {
    try {
      const { oldPassword, newPassword } = data;

      await this.authService.changePassword(`${req.currentUser?.id}`, oldPassword, newPassword);
